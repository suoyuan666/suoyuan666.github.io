...
有一点我认为比较难绷的是，GNOME 的设置里没有分数缩放这个功能，屏幕的缩放只能是整数的，只能在*优化*这个软件（软件包名应该叫 gnome-tweaks，有的发行版可能有个 gnome3-前缀）。我不好评价 gnome-tweaks 那里的那个选项该不该认为是分数缩放，但反正，差不多吧。（可以使用 gsettings 开启实验性的分数缩放支持，该分数缩放会让 XWayland 的缩放变的模糊）。
