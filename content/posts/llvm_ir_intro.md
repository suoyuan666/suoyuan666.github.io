...
> - 2026 03 10 更新:
>     - 因为我给 LLVM 提 PR，所以我特意使用 Debug 模式编译。我发现构建目录占用也不是很大
> - 2026 04 23 更新:
>     - .bin 这个位置太怪了，改一下
> - 2026 06 16 更新:
>     - 删除 `cmake --build build -t install` 时的 `sudo`，我明明设置的位置是 `$HOME/` 下的，要什么 `sudo`...
>     - 同时增加一下使用编译后 clang 的说明，毕竟我们自定了安装目录
> - 2026 06 25 更新:
>     - 将 LLVM 编译配置的 `-DLLVM_TARGETS_TO_BUILD="X86;RISCV"` 改为 `-DLLVM_TARGETS_TO_BUILD="Native"`，`Native` 和 `host` 都用来指代当前主机对应的架构。我曾经还加入了 `RISC-V` 是本以为我可以开始看看开始对编译器后端的学习，我觉得 RISC-V 是一个不错的目标就加进去了，可惜目前我主要集中在 Clang 前端的静态分析中。本系列计划第三章详细介绍各个经典 Pass，对于此计划还没有排期（
...
$ cmake -S llvm -B build -G Ninja           \
        -DCMAKE_INSTALL_PREFIX=~/.llvm_out/ \
...
$ cmake --build build -t install # $HOME/.llvm_out/
```
...
> 后来我使用 Debug 模式重新编译了，构建目录也只是占用了 21 GB，所以看起来还好，可能是我使用了 DYLIB 的原因导致没到我听说的 200 GB 的恐怖占用。

我为什么特意设置 `CMAKE_INSTALL_PREFIX`，纯粹是因为默认是 /usr/local/bin/ 的话有些麻烦

这样安装后，你需要在使用时设置两个环境变量才可以:

```bash
export PATH="$HOME/.llvm_out/bin/:$PATH"
export LD_LIBRARY_PATH="$HOME/.llvm_out/lib/:$LD_LIBRARY_PATH"
```

因为这个 `.llvm_out` 并不是什么正常目录。
