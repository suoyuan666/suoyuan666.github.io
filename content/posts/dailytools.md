...
[PowerToys](https://github.com/microsoft/PowerToys) 是一个 Microsoft 开源的一个使用工具集合，我比较喜欢窗口置顶以及键盘重映射功能。自从我使用了 neovim 后，我已经把我 GNU/Linux 系统这边的 CapsLock 这个按键改成了 Esc，所以为了习惯，Windows 这边我也需要这个修改。窗口置顶则是因为我笔记本电脑屏幕太小，平铺两个窗口不合适，但是对照者看的时候还有些麻烦，所以就需要这个了。
...
[neovim](https://github.com/neovim/neovim) 是我目前使用的编辑器，我最开始使用它的原因其实是因为当时的 vscode 对 wayland 的支持还不是很完美，所以我选择了使用 neovim，毕竟它是跑在终端模拟器里，而不是一个 GUI 程序。

[Bear](https://github.com/rizsotto/Bear) 可以生成一个 `clang` 工具链解析的编译记录文件。用来让 vscode 的 `clangd` 解析并理清项目的编译依赖关系。

[strace](https://github.com/strace/strace) 可以 trace 系统调用，虽然我其实用的不多，不过偶尔还是有些用处的（比如我曾经好奇 `neofetch` 是从哪里 fetch 到这些信息的，但是我又懒得去看源代码，于是就 trace 了一下 neofetch 的系统调用，看看它 open 了哪些文件）。相应的还有 `ltrace`，`ftrace` 什么的，前者可以 trace 动态链接库函数的调用，后者 trace 的是内核函数好像。不过这俩个我就没怎么用过了。
...
[mpv](https://github.com/mpv-player/mpv) 是一个可配置选项很多的多媒体播放器，我很喜欢使用它，因为我认为 mpv 的界面很纯粹，不过这个纯粹的界面完全是因为 mpv 的配置需要写配置文件而不是手动在它的 GUI 程序上配置。

### Input Method

[fcitx5](https://github.com/fcitx/fcitx5) 是一个输入法框架。。fcitx5 我会搭配 [fcitx5-rime](https://github.com/fcitx/fcitx5-rime) 使用，因为我倾向于用[雾凇拼音](https://github.com/iDvel/rime-ice)。不过很多人喜欢使用 fcitx + 搜狗输入法，但是搜狗仍不支持 fcitx5。
...
[KeePassXC](https://keepassxc.org/) 是一个本地存储的密码管理器，在一些面向追求隐私和安全群体的社区中很有热度。
...
[Aegis](https://github.com/beemdevelopment/Aegis) 是一个开源的 2FA Android 客户端，和 Google 2FA 的区别在于它没有云同步。
...
[微信](https://weixin.qq.com/)这个 B 软件我是真不想用，但又不得不用。

[QQ](https://im.qq.com/index/) 这个B 软件同样也不想用，但是也还是得用。
